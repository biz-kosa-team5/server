...
def region_markers(session: Session, payload: dict[str, Any]) -> list[dict[str, Any]]:
  ...
  return [
    ...
    for row in regions_in_bounds(session, bounds, region_type)
  ]
...
def region_type_from_payload(payload: dict[str, Any]) -> str:
  value = payload.get("region")
  if not isinstance(value, str) or value not in SUPPORTED_REGION_TYPES:
    raise ValueError("Unsupported region marker type")
