...
def build_simple_lookup_tool(session: Session):
  ...
  def simple_lookup(
    ...
    """
    아파트 단지의 위치, 주소, 실거래 내역, 특정 단지의 최고가/최저가 같은 단순 조회 질문을 처리합니다.

    "얼마야", "요즘 얼마야", "최근 얼마야", "시세 알려줘", "최근 실거래가", "거래내역"처럼
    현재 가격이나 최근 거래를 묻는 특정 단지 질문은 query_type="trade"로 처리하세요.

    단, "추이", "흐름", "변화", "상승률", "하락률" 같은 변화 분석 표현이 함께 있으면
    simple_lookup이 아니라 price_trend 담당입니다.

    특정 단지의 최고가/최저가 질문도 query_type="trade"로 처리하고 price_order를 함께 전달하세요.
    "은마 최고가", "반포자이 가장 비싼 거래", "잠실엘스 제일 비싼 실거래가"는 price_order="highest"입니다.
    "은마 최저가", "반포자이 가장 싼 거래", "잠실엘스 제일 싸게 거래된 실거래가"는 price_order="lowest"입니다.
    price_order에는 기본값을 두지 마세요. 최고가/최저가 표현이 있을 때만 채우세요.

    지역 내 가격 순위 질문은 simple_lookup에서 처리하지 마세요.
    다음 질문은 price_trend 담당입니다:
    - "강남구 최고가 아파트 TOP 5"
    - "서초구에서 가장 비싼 아파트 5곳"
    - "송파구 최저가 아파트 5곳"
    - "강남구에서 가장 싼 아파트 TOP 5"
    - "강남구 상승률 높은 아파트 TOP 5"
    - "서초구 하락률 높은 아파트 5곳"
    ...
    Args:
      ...
      price_order: 조회 조건 내 특정 단지 최고가/최저가 거래 정렬 방향입니다. highest 또는 lowest 중 하나입니다.
        특정 단지 최고가 질문은 highest이고 특정 단지 최저가 질문은 lowest입니다.
