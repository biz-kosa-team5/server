...
from app.chatbot.features.price_trend.policy import (
...
def run_price_trend(session: Session, slots: dict[str, Any], _: str = "") -> dict[str, Any]:
  try:
    request = TrendSlots(**slots)
  ...
  result = TrendService(PriceTrendDao(session)).handle(request).model_dump(mode="json")
  return {"handler": "price_trend", **result}
