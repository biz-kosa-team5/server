...
TRANSPORT_KEYWORDS = ("역세권", "지하철", "지하철역", "전철역", "교통", "역 근처", "역 주변", "역 인근")
...
def extract_recommendation_slots(question: str) -> dict[str, Any]:
  ...
  if is_child_friendly_query(text) and school_name is None and not school_types:
    ...
    slots["radius_m"] = slots.get("radius_m") or DEFAULT_RADIUS_M
    slots["sort_by"] = "school_distance_asc"
  elif is_specific_education_cluster_query(text) and school_name is None and not school_types:
...
def extract_neighborhood_name(text: str) -> str | None:
  condition_match = re.search(
    r"(?<![가-힣A-Za-z0-9])([가-힣A-Za-z0-9]{2,}동)\s*(?:의|에|에서|쪽)?\s*(?=(?:지하철|전철|역|학교|학군|초등|중학교|고등|유치원|병원|마트|상권|생활편의|인프라|아파트|단지|추천|근처|주변|인근))",
    text,
  )
...
def is_specific_education_cluster_query(text: str) -> bool:
  return (
    is_child_friendly_query(text)
    or "학군" in text
    or "초품아" in text
...
def looks_like_generic_station_reference(text: str) -> bool:
  ...
  return cleaned in {"역", "지하철", "지하철역", "전철", "전철역", "철도역"}
